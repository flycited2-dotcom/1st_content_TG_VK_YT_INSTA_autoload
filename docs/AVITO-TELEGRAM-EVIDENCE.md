...
- Bot API getMe confirmed `Sendpr1ce_bot`, id ***масковано***.
- getChat confirmed review id `-542…2212` (маска: репозиторий публичный), group title `Тест_контент_завод`.
- getChat confirmed publish id `-1002…0509` (маска: репозиторий публичный), channel title `СплитХаб.ру (SplitHub.ru)`.
