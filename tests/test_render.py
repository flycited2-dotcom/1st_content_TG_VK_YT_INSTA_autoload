...
def test_caption_price_on_own_line_with_emoji():
    # выбор владельца 2026-07-05: цена — отдельной плашкой-цитатой (blockquote),
    # 💎 + жирный номинал
    cap = render_caption(_offer(), 25990, CFG)
    lines = cap.splitlines()
    assert "25 990" not in lines[0]                 # первая строка — только название
    assert lines[1] == "<blockquote>💎 <b>25 990 ₽</b> · 1 шт.</blockquote>"
...
def test_storefront_availability_sentinel_is_not_shown_as_quantity():
    o = _offer(sku="storefront:A-1", model="AVS 8000D 8000ВА 40А КПД 98% циф. индикация")
    o.source = "storefront"
    o.attrs = {"Гарантия": "12 мес"}
    cap = render_caption(o, 15990, CFG)
    assert "1 шт." not in cap
    assert "8000 ВА" in cap and "40 А" in cap and "КПД: 98%" in cap
    assert "Гарантия: 12 мес" in cap
...
def test_serial_caption_header_without_article_and_from_price():
    ...
    assert price_line == "<blockquote>💎 <b>от 22 390 ₽</b></blockquote>"   # цена — плашкой-цитатой
...
def test_serial_caption_falls_back_when_single_model():
    ...
    assert cap.splitlines()[1].startswith("<blockquote>💎 <b>22 390 ₽</b>")   # одна модель → обычный формат
